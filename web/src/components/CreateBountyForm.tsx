...
/** Default datetime-local value = now + offset hours, in the input's expected format. */
function defaultDeadline(offsetHours: number): string {
...
export function CreateBountyForm({ onCreated }: { onCreated?: (bountyId: bigint) => void }) {
  ...
  const validation = useMemo(() => {
    ...
    if (!Number.isFinite(submissionMs)) return "Invalid submission deadline.";
    if (!Number.isFinite(revealMs)) return "Invalid reveal deadline.";
    if (revealMs <= submissionMs) return "Reveal deadline must be after submission deadline.";
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const submissionDeadlineTs = BigInt(Math.floor(submissionMs / 1000));
  ...
  return (
    <Card>
      <CardHeader
        ...
        subtitle="Fund a reward, then set commit and reveal deadlines."
      />
      <CardBody>
        ...
        <form onSubmit={handleSubmit} className="mt-3 space-y-3">
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            <Field label="Submission deadline" hint="Commitments only before this time.">
              <Input
                type="datetime-local"
            ...
            <Field label="Reveal deadline" hint="Plain answers + salts are revealed before this time.">
              <Input
                type="datetime-local"
